...
export default function Contact() {
  ...
  return (
    <section
      id="contact"
      className="py-36 px-14 max-w-[1200px] mx-auto"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-20 items-start">
        ...
        <div>
          <SectionReveal>
            ...
            >
              <span
                className="block mb-6 text-[13px] tracking-[0.1em]"
          ...
          <SectionReveal delay={0.1}>
            <p className="text-sm leading-[1.9] mb-10" style={{ color: 'var(--color-muted)' }}>
              設計できる人を探しているなら、<br />それはここです。
            </p>
            <div className="flex flex-col gap-3">
              {SNS.map(({ label, href }) => (
                <a
                  ...
                  className="no-underline flex items-center gap-3 text-[11px] tracking-[0.08em] transition-colors duration-200 group"
                ...
                >
                  <span className="w-4 h-px transition-all duration-200" style={{ background: 'var(--color-border)' }} />
                  {label}
                </a>
              ))}
            </div>
        ...
        <SectionReveal delay={0.15}>
          {status === 'sent' ? (
            <div
              ...
              style={{ fontFamily: 'var(--font-shippori)', fontSize: '18px', color: 'var(--color-taupe)' }}
            >
              送信しました。<br />
              <span className="text-[13px]" style={{ color: 'var(--color-muted)' }}>
                ありがとうございます。折り返しご連絡します。
              </span>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="flex flex-col gap-8">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="ご依頼・ご質問などお気軽にどうぞ"
                />
              </div>
              <button
                type="submit"
                disabled={status === 'sending'}
                className="self-start transition-all duration-250 disabled:opacity-50"
                style={{
                  fontFamily: 'var(--font-dm-mono)',
                  fontSize: '12px',
